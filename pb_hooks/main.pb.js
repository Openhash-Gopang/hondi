...
routerAdd("POST", "/api/tx", (c) => {
  ...
  // 매번 새로 만든다. ([2026-09-19] 여기서 예고했던 대로 _balanceUtils와
  // onRecordBeforeUpdateRequest(profiles PATCH)가 쓰는 _sigVerify 둘 다
  // 실제로 같은 버그를 안고 있었던 것이 확인·수정 완료됐다 — 각각 콜백
  // 내부 지역 재선언으로 전환. 이 파일에서 top-level 선언을 콜백 안에서
  // 그대로 참조하는 패턴은 이제 남아있지 않다.)
var _sigVerify = (function() {
var nacl = {};
'use strict';
...
onRecordBeforeUpdateRequest((e) => {
  ...
  // [2026-09-19] 이 훅도 위 _balanceUtils와 동일한 이유로 top-level
  // _sigVerify(파일 최상단 line 16) 참조가 콜백 안에서
  // "_sigVerify is not defined"로 깨지는 것을 확인했다 — /api/tx 훅에서
  // 이미 검증된 워크어라운드(콜백 내부 지역 재선언)를 여기도 그대로
  // 적용한다. /api/tx의 로컬 _sigVerify(이 파일 line 1237 부근)와 완전히
  // 동일한 TweetNaCl 포트다 — 두 훅에서 각각 지역 선언해야 하는
  // Goja 제약 때문에 부득이하게 중복시켰다.
var _sigVerify = (function() {
var nacl = {};
'use strict';
