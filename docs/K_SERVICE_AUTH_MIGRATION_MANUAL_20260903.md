...
> ⚠️ **2026-09-17 갱신 주석** — 이 문서에서 설명하는 "개별 함수 안에서
> `phone_verify_token`을 검증하는" 방식(모델 B, §2)은 폐기됐다.
> klaw/kplan/kjit/kcity/gov는 이제 디스패치 지점 단 한 곳의 공용
> 게이트("현관문 하나")에서 인증하고, 개별 핸들러는 이미 검증된
> `body.guid`를 그대로 신뢰한다 — 새로 작성하는 코드는 이 문서의
> §3-B 레시피가 아니라 **`docs/HONDI_UNIFIED_AUTH_BILLING_MANUAL_v1_0_20260917.md`
> §5 체크리스트**를 따를 것. 이 문서는 2026-09-03 시점의 작업 기록
> (지갑 서명 전용 27곳 스캔 결과 등)으로서 그대로 보존한다.
