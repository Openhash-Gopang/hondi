...
migrate((db) => {
    ...
    for (const record of batch) {
      ...
      try {
        ...
        if (oldHash === newHash && record.getString("e164_last8") === newLast8) {
          ...
          console.log(`[E164-RESYNC] guid=${record.getString("guid")} 해시 갱신: ${oldHash ? oldHash.substring(0, 8) + "…" : "(없음)"} → ${newHash.substring(0, 8)}…`);
        }
        ...
        totalFailed++;
        console.log(`[E164-RESYNC] guid=${record.getString("guid")} 실패(건너뜀): ${err.message}`);
      }
    }
  ...
  console.log(`[E164-RESYNC] 완료 — 처리 ${totalProcessed}건(값 변경 ${totalProcessed - totalUnchanged}건, 동일 ${totalUnchanged}건), 실패 ${totalFailed}건`);
}, (db) => {
  ...
  console.log("[E164-RESYNC] 되돌리기 없음 — 이 마이그레이션은 해시를 현재 비밀값 기준으로 갱신할 뿐이라 이전 상태를 복원할 수 없습니다.");
});
