...
# 문서 코드  : AC-EVOLUTION
# 버전      : v1.5 (v1.4 대체)
# v1.5 변경 요약 — "#18 진행하세요" 지시(2026-07-14, 7차):
#   - #18 완결: docs/ACCESS_CERT_v1_0.md 신설. 조사 중 authoritativeAgency
#     가 클라이언트 자칭 문자열을 그대로 신뢰하던 실제 보안 결함을
#     발견(주피터님 "저장소를 검색해보세요" 지시로 확인) — 기존 Ed25519
#     TOFU 인프라(business/citizen용)를 기관에도 확장해 서명 기반
#     access_cert로 교체. _authoritativeCheck를 느슨한 접두어 매칭에서
#     정확 일치로 재설계. DEPT_TASK_REQUEST의 _verifyEd25519 스텁
#     버그도 함께 수정. AGENCY_PUBKEY_REGISTRY는 빈 채로 배포(최초
#     기관 키 등록은 여전히 저장소 밖 절차).
# v1.4 변경 요약 — "#15 진행" 지시(2026-07-13, 6차):
#   - #15 완결: GOV_TASK 병렬·팬아웃 처리 모델 신설
#     (docs/GOV_TASK_FANOUT_v1_0.md). "병렬"을 물리적 동시실행이
#     아니라 "순서 의존관계 없음"으로 재정의, notify/join 두 유형.
#     worker.js(batch_id/fanout_mode, handleGovTaskBatchStatus)·
#     SP-20 STEP 4-P·SP-21 STEP 1 반영. GAPS #15 항목 참고.
...
- **#18(최초 관리자 임명)은 시스템이 아니라 법령이 해법이다** (주피터님
  ...
  **★ 2026-07-14 정정** — 위 결론의 전제("시스템의 책임은 그 세션
  인증을 정확히 신뢰하는 것까지")가 실제로는 **성립하지 않고 있었다.**
  `authoritativeAgency`가 그때까지 클라이언트가 요청 본문에 적어 보낸
  `agency`/`business_id` 문자열 그대로였다 — "세션 인증"이라 부를 만한
  게 실제로는 전혀 없었다. `docs/ACCESS_CERT_v1_0.md`가 이 간극을
  메웠다 — 기존 Ed25519 TOFU 인프라(business/citizen용)를 재사용해
  기관장 서명+본인 서명 이중 검증(access_cert)으로 교체했다. "그
  세션에 누가 들어올 자격이 있는지"가 여전히 국가/기관 인사 시스템의
  몫이라는 결론 자체는 맞았지만, "시스템이 세션 인증을 정확히
  신뢰한다"는 전제가 실제로 지켜지지 않고 있었다는 게 이번에 드러난
  차이다.
...
- **#15 — 완결.** `docs/GOV_TASK_FANOUT_v1_0.md` 신설 — GAP-LIST-50
  B-3(여러 부서 동시 협의·한 사건 여러 기관 통지 처리 모델 부재)를
  해소했다. 핵심은 "병렬"의 재정의다 — 이 시스템은 턴 단위 LLM
  호출이라 물리적 동시실행이 불가능하므로, "병렬" = "순서 의존관계
  없음"으로 정의하고 notify(통지형, 서로 독립)/join(협의형, 전원
  완료 필요) 두 유형으로 나눴다. `gov_task_submission`에
  `batch_id`/`fanout_mode` 필드만 추가(신규 컬렉션 없음, 하위호환
  100%), `handleGovTaskBatchStatus`로 집계 조회, K-Compose STEP 4-P·
  K-Deliver STEP 1에 실행/보고 로직 반영. **한계**: join 모드의
  "거부" 판정 배선이 아직 없어(기관이 명시적으로 거부하는 액션 자체가
  구현 안 됨) 지금은 전원 승인 전까지 계속 in_progress로만 보인다 —
  후속 과제로 명시(GOV_TASK_FANOUT §6).

## 7. 이 문서가 여전히 확정하지 않는 것

- **#18의 잔여분**: `AGENCY_PUBKEY_REGISTRY`가 빈 채로 배포됐다 —
  실제 기관 공개키(기관장 서명키) 확보·등록은 여전히 이 저장소 밖의
  절차이고, 그 전까지 모든 기관의 privileged 기능(소속 승인·PDV
  요청·업무지시)은 실질적으로 비활성 상태다(안전한 기본값). 공무원
  개인이 자기 키를 어떻게 발급받고 보관하는지의 UX도 미정
  (`docs/ACCESS_CERT_v1_0.md` §4).
- #15의 join 모드 "거부" 판정 배선 — GOV_TASK_FANOUT §6에 한계로
  명시된 채 남아 있다(후속 과제).
...
---
*v1.5 (2026-07-14) — #18 완결. `docs/ACCESS_CERT_v1_0.md` 신설 —
authoritativeAgency가 클라이언트 자칭 문자열을 그대로 신뢰하던 실제
보안 결함을 발견·수정(기존 Ed25519 TOFU 인프라를 기관까지 확장,
기관장 서명+본인 서명 이중 검증). `_authoritativeCheck`를 정확 일치로
재설계, `DEPT_TASK_REQUEST`의 서명검증 스텁 버그도 수정. GAPS 22건
전부 완결 또는 조치불필요 — 잔여분은 저장소 밖 절차(기관 키 등록)뿐.
v1.4 (2026-07-13) — #15 완결. GOV_TASK 병렬·팬아웃 모델 신설
(docs/GOV_TASK_FANOUT_v1_0.md) — "병렬"을 순서 의존관계 없음으로
재정의, notify/join 두 유형, batch_id/fanout_mode를 기존
gov_task_submission에 무손실 추가. join의 "거부" 판정 배선은
후속 과제로 열어둠.
