...
## 작업 #22 (2026-09-26) — 남은 미해결 항목 진행: ARTMUSEUM/HERITAGE 수정, B4 지방노동위원회 해소, B5/B6 추가 시도

프로젝트 총괄 지시: "미해결 항목을 진행하세요." 작업 #21에서 남긴 4개 항목(ARTMUSEUM/HERITAGE
고유명사 위험, B4·B5·B6) 전부를 이어서 진행했다.

### ARTMUSEUM/HERITAGE 고유명사 division 라우팅 위험 — 수정 완료

작업 #21 A3에서 발견만 하고 범위 밖이라 미루었던 두 기관을 이번에 고쳤다. "문화사랑회"·
"서귀포고용센터"와 정확히 같은 클래스(고유명사·시설명이 division kw에만 있고 기관 kw에는
없는 구조) — `SP-AGY-ARTMUSEUM` 기관 kw에 "제주현대미술관"·"김창열미술관"을, `SP-AGY-
HERITAGE` 기관 kw에 "한라산국립공원"·"한라수목원"을 추가했다(HERITAGE는 desc에는 이미
"한라산국립공원"이 있었지만 kw 배열 자체엔 없어 결정론적 매칭 단계에서는 못 잡았다).
오프라인 구조 검증(classifyFn 없이도 4개 시나리오 전부 즉시 확정)으로 확인 완료 — 회귀 없음
(기존 division kw 그대로 유지).

### B4. 지방노동위원회 소속 — 결정적 근거 추가 확보, 라우팅 수정 완료

law.go.kr·jeju.go.kr 직접 접속은 이번에도 robots.txt로 막혔지만, **ulex.co.kr(국가법령정보
미러)** 경유로 처음으로 원문급 근거를 확보했다: 「제주특별자치도 행정기구 설치 및 정원
조례」 제3조(합의제행정기관) 전문 —

> "도지사 소속하에 그 직무에 관하여 독립된 지위를 가지는 제주특별자치도감사위원회 및
> 제주특별자치도지방노동위원회를 둔다."

이 조례 제1조(목적)는 「제주특별자치도 설치 및 국제자유도시 조성을 위한 특별법」 제10·15·
23·24·25·44·88·131·341·405·453조를 근거 법령으로 명시한다 — 즉 **보훈청과 정확히 같은
클래스**: 제주특별법의 특례에 따라 지방노동위원회도 도지사 소속 합의제행정기관으로 실제
설치돼 있다고 볼 근거가 생겼다. 감사위원회가 도 소속이라는 데는 애초에 이견이 없었다는
점도 방증이 된다(같은 조문, 같은 문장 구조로 나열돼 있다).

**수정**: 보훈청 때와 정확히 같은 클래스의 구조적 버그를 발견해 함께 고쳤다 —
`_localGovCollisionCandidate()`가 agency 테이블만 검사하고 **collegial 테이블은 검사하지
않아서**, `SP-NAT-LABORREL`의 바레 `'노동위원회'` 키워드가 `SP-COMM-LABOR`와의 충돌 감지도
없이 0단계에서 즉시 확정돼버리는 구조였다(작업 #19에서 agency만 고치고 collegial은 놓친
사각지대). agency와 동일한 패턴으로 collegial 테이블도 후보에 포함시켰다. `ROUTE_
DESCRIPTIONS['SP-NAT-LABORREL']`도 정정 — "고용노동부 소관 원칙(전국 공통)이지만 제주는
조례 제3조에 따라 도지사 소속으로 별도 설치돼 있어 실제로는 도 소관일 가능성" 명시.

오프라인 mock LLM으로 구조 검증: "지방노동위원회에 부당해고 구제신청 하고 싶어요" →
수정 전엔 국가기관(SP-NAT-LABORREL)으로 즉시 확정(경합 감지 자체가 없었음), 수정 후엔
`(계층충돌 통합판단 — 지방행정(SP-COMM-LABOR) 소관으로 판정)` 경로로 LLM에게 두 후보
(SP-NAT-LABORREL의 정정된 설명 포함)를 제시하고 판단하게 한다 — classifyFn 없는 오프라인
기본값은 안전하게 국가기관 유지(회귀 없음).

**완전히 해소된 것은 아니다** — (1) law.go.kr 원문(제주특별법 해당 조항 자체)은 여전히
직접 대조 못했다(ulex.co.kr은 조례 제3조까지만 확인, 특별법 원문 조항 번호가 정확히 무엇을
규정하는지는 못 봄). (2) "위원회 자체"와 이를 지원하는 "사무국"이 조례상 구분되는지도
미확정. (3) `노동위원회법` 제2조(전국 공통, 고용노동부 소속)와의 관계도 여전히 남아있다 —
제주특별법이 이 조항의 특례를 명시적으로 규정하는지까지는 확인 못했다. 다음 배치에서 law.go.kr
원문 재시도 필요.

### B5·B6 — ulex.co.kr로 추가 시도했으나 여전히 미확보

같은 ulex.co.kr 경로로 "제주특별자치도 행정기구 설치 조례 시행규칙"(117개 조항)과
"제주특별자치도 감사위원회 구성 및 운영에 관한 조례"에 접근은 됐다. 다만 이번엔 접근 자체가
아니라 **웹 조회 도구가 긴 문서의 앞부분(제1~5조)만 반환하고, 자치경찰위원회·감사위원회
사무기구가 다뤄질 것으로 추정되는 후반부(제60조 이후)는 반환하지 못하는** 새로운 종류의
한계에 부딪혔다 — 감사위원회 조례 제13조는 "사무국의 조직과 정원 및 직무 등에 대하여는
「제주특별자치도 행정기구 설치 및 정원 조례」 등에 따른다"고 위임할 뿐이라, 결국 필요한
정보는 같은 시행규칙 후반부에 있다. "자치경찰협력과"·"심의과"·"부패방지지원센터" 문자열로
웹 검색도 재시도했으나 스니펫에 잡히는 곳이 없었다. **B5(자치경찰협력과 명칭)·B6(감사위원회
division 구성) 모두 여전히 미확정** — org-baseline-collegial.json의 open_questions에 이번
시도 내역을 추가 기록했다.

## 완전한 검증을 위해 남은 일 (작업 #22)

- ARTMUSEUM/HERITAGE 수정·B4 라우팅 수정 모두 오프라인 회귀 스윕으로만 확인했다 — 병합 후
  `live-smoketest-gov-router-2026-09-24-new-agencies-broad.yml`류에 이번에 고친 시나리오
  (김창열미술관·제주현대미술관·한라산국립공원·한라수목원·지방노동위원회)를 추가해 재실행하면
  실측 검증이 가능하다.
- B5·B6은 원문(제주특별자치도 행정기구 설치 조례 시행규칙 제60조 이후)을 반환할 수 있는 다른
  조회 경로(예: 직접 PDF 확보, 또는 도청 정보공개청구)가 필요하다 — 웹 조회 도구 한계로 이번
  세션에서는 더 진전이 어렵다.
