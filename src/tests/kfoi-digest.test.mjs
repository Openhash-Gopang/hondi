...
await test('직속기관·사업소(agency) 조직 기준표: 2026-09-22까지 6개(농업기술원·보건환경연구원·축산생명연구원·한라도서관·자치경찰단·민속자연사박물관)는 match로, 나머지 3개는 여전히 structure_differs로 정직하게 남는다', () => {
  const t = digest.tiers.agency;
  assert.equal(t.baseline.org_counts.structure_differs, 3);
  assert.equal(t.baseline.org_counts.match, 6);
